...
router.get("/", auth, async (req, res) => {
  try {
    const hcpDatalist = await hcpPerDetSchema.find({ PharmaCompany: req.user.id }).sort({
      date: -1
    });
...
router.post("/", auth, async (req, res) => {
  ...
  try {
    const user1 = await User.find({ email : hcpEmail }).sort({date: -1});
...
router.put("/:id", auth, async (req, res) => {
  ...
  if (hcpEmail) detailFields.hcpEmail = hcpEmail;
